...
//    echo "</pre>";
        foreach ($Survey as $Val) {
            if (trim($Val['ab']) != '' && trim($Val['x']) != '' && trim($Val['ab'] != '')) {
                $Iterator++;
...
//                $DataArray['data'][$Val['b']][$Iterator][] = $Val['e'];
                if (isset($DataArray['data'][$Val['ab']][$Val['x']])) {
                    if (!in_array($Val['y'], $DataArray['data'][$Val['ab']][$Val['x']])) {
                        $DataArray['data'][$Val['ab']][$Val['x']][] = $Val['y'];
                    }
                } else {
                    $DataArray['data'][$Val['ab']][$Val['x']][] = $Val['y'];
                }
                ...
                if (!isset($DataArray['total'][$Val['ab']][$Val['y']][$Val['e']])) {
                    $DataArray['total'][$Val['ab']][$Val['y']][$Val['e']] = 0;
                }

                $DataArray['total'][$Val['ab']][$Val['y']][$Val['e']] = $DataArray['total'][$Val['ab']][$Val['y']][$Val['e']] + 1;
...
" id="btn{$ConvertedKey}-hide" href="http://unice.fr/formation/formation-initiale/{$Link[0]['Diplome_code']}{$Link[0]['Version_diplome_code']}" target="_blank">Fiche associée sur le site de l'université</a>
...
//        echo "</pre>";
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Durée d’accès à l’emploi', 'Percentage'],
...
hide;
        ...
        var options = {
          title: 'Durée d’accès à l’emploi',
          pieSliceText: 'none'
        };
...
hide;
               ...
               if($LKey=="")   ///condition added by mustafa to skip the empty bars
				    continue;
					$counter++;
                     $Percent = round(ceil (($Val*100)/$ValSum));
                    $HiddenDiv .= <<<hide
...
if($counter>5)
					  {
						   ...
						   $HiddenDiv = " <div class=\"text-container\">
								<div class=\"text-content short-text\">" . $HiddenDiv;
